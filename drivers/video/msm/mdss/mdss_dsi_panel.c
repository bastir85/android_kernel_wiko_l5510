...
void mdss_dsi_panel_pwm_cfg(struct mdss_dsi_ctrl_pdata *ctrl)
{
 ...
 //Begin<20150916><modify for charger in power off>;xiongdajun
#if defined(CONFIG_PROJECT_L5510) || defined(CONFIG_PROJECT_L5320)
extern int enable_clk;
#endif
//END<20150916><modify for charger in power off>;xiongdajun
static void mdss_dsi_panel_bklt_pwm(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
 ...
 //Begin<20150916><modify for charger in power off>;xiongdajun
#if defined(CONFIG_PROJECT_L5510) || defined(CONFIG_PROJECT_L5320)
            if(enable_clk == 2) {   //if send backlight off when power off charger
			ret = pwm_config_us(ctrl->pwm_bl, level,
					ctrl->pwm_period);
			if (ret)
				pr_err("%s: pwm_config_us() failed err=%d.\n",
							...
							u32 bl_level)
{
	...
	//<BEGIN><><20150330><modify brightness for l9010>xiongdajun
	#if defined(CONFIG_PROJECT_L9010)	
	struct mipi_panel_info *mipi;
	struct mdss_dsi_ctrl_pdata *ctrl = NULL;
	#endif
	//<END><><20150330><modify brightness for l9010>xiongdajun
	if (pdata == NULL) {
		pr_err("%s: Invalid input data\n", __func__);
		return;
	}
...
//<BEGIN><><20150401><add gpio esd check for l9010>xiongdajun
#if defined(CONFIG_PROJECT_L9010)	
	ctrl_pdata->check_esd_gpio  = of_get_named_gpio(np,
				"qcom,mdss-dsi-esd-gpio", 0);

		pr_debug(" gpio is %d",ctrl_pdata->check_esd_gpio);
	if (!gpio_is_valid(ctrl_pdata->check_esd_gpio))
	{
		pr_debug("%s:%d, check esd gpio is not valid\n",
						__func__, __LINE__);
		goto error;
	}
	else{
		pr_err("%s:%d, check esd gpio  specified\n",
						__func__, __LINE__);
		}
	tmp = gpio_request(ctrl_pdata->check_esd_gpio, "check_esd_n");
	if (tmp) {
		pr_err("request check gpio failed, rc=%d\n",
			tmp);
		goto error;
		...
		{
			pr_err("request check gpio  OK, rc=%d\n",
			tmp);
	}
#endif	
//<END><><20150401><add gpio esd check for l9010>xiongdajun
...
//BEGIN <LCM> <DATE20150203> <add TE check for lcm eds check> limi.zhang
 	rc = of_property_read_string(np, "qcom,mdss-dsi-TE-mode", &data);
 	if (!rc) {
 		if (!strcmp(data, "on"))
...
//END <LCM> <DATE20150203> <add TE check for lcm eds check> limi.zhang
	...
	if (!rc) {
		...
		//<BEGIN><><20150401><add gpio esd check for l9010>xiongdajun
		#if defined(CONFIG_PROJECT_L9010)
		} else if (!strcmp(data, "gpio_read")) {
			ctrl_pdata->status_mode = ESD_GPIO;
		#endif
		//<END><><20150401><add gpio esd check for l9010>xiongdajun
		}
	}
