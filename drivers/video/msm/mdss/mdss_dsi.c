				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_UNBLANK:
        //Begin<20150916><modify for charger in power off>;xiongdajun
#if defined(CONFIG_PROJECT_L5510) || defined(CONFIG_PROJECT_L5320)
            if(enable_clk){                //if in charger when power off
                enable_clk= 2;          //send backlight 0 cmd
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	#if defined(CONFIG_PROJECT_L9010)
	else if (ctrl_pdata->status_mode == ESD_GPIO)
		ctrl_pdata->check_status = mdss_dsi_gpio_status_check;
	#endif
