...
/**
 * mdss_dsi_gpio_status_check() - Check dsi panel status through gpio value check
 * @ctrl_pdata: pointer to the dsi controller structure
 *
 * This function can be used to check gpio value of the panel using bta check
 * for the panel.
 *
 * Return: 0 value if the panel is in good state, 1 otherwise
 * 
 */
int mdss_dsi_gpio_status_check(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	int ret = 0;
	int temp = 0;
	...
	{
		temp=gpio_direction_input(ctrl_pdata->check_esd_gpio);
		pr_debug(" set input gpio %d\n",temp);
		temp = gpio_get_value(ctrl_pdata->check_esd_gpio);
		pr_debug( " get input gpio %d\n",temp);
		if(temp ==0 )
			ret = 1;
		else ret = -EINVAL;
	}
