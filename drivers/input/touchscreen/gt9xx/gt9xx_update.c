...
//BEGIN <tp upgrade> <DATE20141223> <detect ffbm> limi.zhang
#if defined(GTP_AUTO_UPDATE)
#define GTP_FTM 1
static int gtp_ffbm_flag =0;
static void read_ffbm_flag(void);
#endif
//END <tp upgrade> <DATE20141223> <detect ffbm> limi.zhang
...
static void read_ffbm_flag(void)
{
	...
	else
	{
		...
		if(pffbm)
		{
			gtp_ffbm_flag = 1; // boot ffbm mode
		}
	}
	pr_info("%s: g_ffbm_flag = %d \n", __func__, gtp_ffbm_flag);
...
s32 gup_update_proc(void *dir)
{
    ...
    //pr_info("limi.zhang===>000000 [update_proc]Begin update ......");
    msleep(3000);
    if(0 == gtp_ffbm_flag)
    {
        read_ffbm_flag();
    }
    if(GTP_FTM == gtp_ffbm_flag) // not ffbm mode, do tp upgrade if need 
    {	
		return FAIL;
	}
	//pr_info("limi.zhang===>111111 [update_proc]Begin update ......");
