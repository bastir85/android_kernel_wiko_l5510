...
//LINE<JIRA_ID><DATE20150424><BUG_INFO>zenghaihui
static int g_hall_data = 0;
...
//LINE<JIRA_ID><DATE20150424><BUG_INFO>zenghaihui
	...
	value = gpio_get_value_cansleep(data->gpio);
        ...
        if(value == 0)
        {
            ...
            pr_info("zenghh hall_interrupt_handler near, g_hall_data: %d \n", g_hall_data);
        }
        else
        {
            g_hall_data = 0;
            
            pr_info("zenghh hall_interrupt_handler far, g_hall_data: %d \n", g_hall_data);
        }
...
static int hall_driver_probe(struct platform_device *dev)
{
	...
	int err = 0;
	int irq_flags;
	int ret_device_file = 0; //LINE<JIRA_ID><DATE20150425><BUG_INFO>zenghaihui
	...
	if (dev->dev.of_node) {
        ...
        //LINE<JIRA_ID><DATE20150424><BUG_INFO>zenghaihui
	ret_device_file = device_create_file(&dev->dev, &dev_attr_hall_data);
	...
	}//<20150728><init hall status>wangyanhui
