...
#include <linux/of_device.h>//start add by xiongdajun 20140305	tinno charge temp manage

//yangliang add to shutdown of low-capacity=0%-voltag<3.4V with external power start;20150716
...
struct smb1360_chip {
	...
	bool				pulsed_irq;
//Begin<REQ><JABALL-1500><20150709>add error hand for smb1360 charge when 45dec;xiongdajun
	int				voltage_now;
//END<REQ><JABALL-1500><20150709>add error hand for smb1360 charge when 45dec;xiongdajun
...
//start add by xiongdajun 20140305	tinno charge temp manage
...
//end add by xiongdajun 20140305	tinno charge temp manage
...
static int64_t float_decode(u16 reg)
{
	...
	pr_info("exponent=%d mantissa=%d sign=%d\n", exponent, mantissa, sign);
...
static enum power_supply_property smb1360_battery_properties[] = {
	...
	POWER_SUPPLY_PROP_TECHNOLOGY,//<FFBFKKA-58><20150107><battery is li-ion>xusong
};
...
//Line<REQ><JABALL-1500><20150709>add error hand for smb1360 charge when 45dec;xiongdajun
...
static int smb1360_get_prop_batt_capacity(struct smb1360_chip *chip)
{
	...
	int charger_error,battery_OV;//LINE<REQ><JABALL-1500><20150709>add error hand for smb1360 charge when 45dec;xiongdajun
	...
	if (temp > (MAX_8_BITS / 2))
		soc += 1;
//Begin<REQ><JABALL-1500><20150709>add error hand for smb1360 charge when 45dec;xiongdajun
	smb1360_read(chip, 0x56, &reg);
	charger_error= (reg&0x04); //read 'charger error' status and mask others
	smb1360_read(chip, 0x53, &reg);
	battery_OV= (reg&0x40); //read 'battery_OV' status and mask others
	chip->voltage_now = smb1360_get_prop_voltage_now(chip);
	pr_err("chip->voltage_now = %d\n", chip->voltage_now);
//Begin<REQ><JABALL-1500><20150713>modify recharger vol;xiongdajun
	if(charger_error && battery_OV&& ( (chip->voltage_now < 4000000) || ( ! (chip->batt_warm || chip->batt_cool)) ) )
	{
	//reset the charger enable bit
	__smb1360_charging_disable(chip, true); // disable charging
	__smb1360_charging_disable(chip, false); // disable charging
	}
//END<REQ><JABALL-1500><20150709>add error hand for smb1360 charge when 45dec;xiongdajun
	pr_info("msys_soc_reg=0x%02x, fg_soc=%d batt_full = %d\n", reg,
						soc, chip->batt_full);
...
static int smb1360_get_prop_chg_full_design(struct smb1360_chip *chip)
{
	...
	pr_info("reg[0]=0x%02x reg[1]=0x%02x fcc_mah=%d\n",
				reg[0], reg[1], fcc_mah);
...
static int smb1360_get_prop_batt_temp(struct smb1360_chip *chip)
{
	...
	pr_info("reg[0]=0x%02x reg[1]=0x%02x temperature=%d\n",
					reg[0], reg[1], temp);
...
static int smb1360_get_prop_voltage_now(struct smb1360_chip *chip)
{
	...
	int cap_zero = 0;
	int chg_status = 0;
	static int count_vol = 1;//10 to 5 to 1;yangliang
	static int vol_sum =0;
	static int vol_aver =0;
	//yangliang add to shutdown of low-capacity=0%-voltag<3.4V with external power end;20150716
	...
	pr_info("reg[0]=0x%02x reg[1]=0x%02x voltage=%d\n",
				reg[0], reg[1], temp * 1000);

	//yangliang add to shutdown of low-capacity=0%-voltag<3.4V with external power start;20150716
	count_vol--;
	vol_sum += temp;
	//printk(KERN_ERR"yangliang acquire-out count_vol=%d\n",count_vol);
	if(count_vol == 0){
		//printk(KERN_ERR"yangliang acquire-in count_vol=%d\n",count_vol);

		vol_aver = vol_sum/1;//10 to 5 to 1;yangliang
		//printk(KERN_ERR"yangliang acquire-in vol_aver=%d\n",vol_aver);
		
		cap_zero = smb1360_get_prop_batt_capacity(chip);
		//printk(KERN_ERR"yangliang acquire-in cap_zero=%d\n",cap_zero);
		
		chg_status = smb1360_get_prop_charging_status(chip);
		//printk(KERN_ERR"yangliang acquire-in chg_status=%d\n",chg_status);
		
		if((cap_zero == 0) && (vol_aver < 3200) && (chg_status == 1)){//3300 to 3600 for testing;yangliang
			//printk(KERN_ERR"yangliang acquire goto out-greater-in-power and shutdown\n");
			printk(KERN_ERR"goto out-greater-in-power and shutdown\n");
			//do_msm_poweroff();
		}else{
			count_vol = 1;//10 to 5 to 1;yangliang
			//count_vol +=1;//10 to 5 to 1;yangliang
			//++count_vol;//10 to 5 to 1;yangliang
...
static int smb1360_get_prop_batt_resistance(struct smb1360_chip *chip)
{
	...
	pr_info("reg=0x%02x resistance=%lld\n", temp, resistance);
...
static int smb1360_get_prop_current_now(struct smb1360_chip *chip)
{
	...
	pr_info("reg[0]=0x%02x reg[1]=0x%02x current=%d\n",
				reg[0], reg[1], temp * 1000);
...
static struct power_supply *get_parallel_psy(struct smb1360_chip *chip)
{
	...
	if (!chip->parallel_psy)
		pr_info("parallel charger not found\n");
	return chip->parallel_psy;
}
...
static void smb1360_parallel_work(struct work_struct *work)
{
	...
	if (rc) {
		pr_info("Unable to read AICL status rc=%d\n", rc);
		goto exit_work;
	}
	pr_info("STATUS_1 (aicl status)=0x%x\n", reg);
	if ((reg & AICL_CURRENT_STATUS_MASK) == AICL_LIMIT_1500MA) {
		...
		pr_info("fast-chg (parallel-mode) current set to = %d\n",
							fastchg_current[i]);
...
static int smb1360_set_appropriate_usb_current(struct smb1360_chip *chip)
{
	...
	if (chip->therm_lvl_sel >= 0				//add by lishengli 20150409 fixed limit current can't set from thermal_mitigation
	...
	pr_info("lishengli current_ma=%d, therm_ma = %d, therm_lvl_sel = %d \n", current_ma, therm_ma, chip->therm_lvl_sel);
							...
							int current_ma)
{
	...
	if (i < 0) {
		pr_info("Couldn't find fastchg_current %dmA\n", current_ma);
		i = 0;
	}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL:
		...
		break;
		//BEGIN<FFBFKKA-58><20150107><battery is li-ion>xusong
			case POWER_SUPPLY_PROP_TECHNOLOGY:
				val->intval = POWER_SUPPLY_TECHNOLOGY_LION;
				break;
		//END<FFBFKKA-58><20150107><battery is li-ion>xusong
	default:
		return -EINVAL;
	}
	return 0;
...
static void smb1360_jeita_work_fn(struct work_struct *work)
{
	...
	g_TinnoBatteryNotifyCode = BATTERY_OVER_TEMPERATURE;
	//end add by xiongdajun 20140305	tinno charge temp manage	
		...
		else chip->tinno_recharge_flag = false;
	...
	//start add by xiongdajun 20140305	tinno charge temp manage	
		...
		chip->tinno_recharge_flag = false;
		chip->tinno_discharge_flag = true;
		g_TinnoBatteryNotifyCode = BATTERY_UNDER_TEMPERATURE;
		//end add by xiongdajun 20140305	tinno charge temp manage	
	...
	if(chip->tinno_recharge_flag == true)
	{
...
static int smb1360_select_fg_i2c_address(struct smb1360_chip *chip)
{
	...
	pr_info("FG_access_type=%d fg_i2c_addr=%x\n", chip->fg_access_type,
							chip->fg_i2c_addr);
							...
							int gain_factor)
{
	...
	if (gain_factor) {
		...
		pr_info("current_gain_reg=0x%x current_gain_decoded=%lld new_current_gain_decoded=%lld new_current_gain_reg=0x%x\n",
		...
		for (i = 0; i < ARRAY_SIZE(reg_val_mapping); i++) {
			...
			pr_info("Writing reg_add=%x value=%x\n",
		...
		pr_info("Disabling gain correction\n");
...
static int otg_oc_handler(struct smb1360_chip *chip, u8 rt_stat)
{
	...
	pr_info("OTG over-current stat=%d\n", rt_stat);
	rc = smb1360_otg_disable(chip);
	if (rc)
		pr_err("Couldn't disable OTG mode rc=%d\n", rc);
...
static int smb1360_check_batt_profile(struct smb1360_chip *chip)
{
	...
	for (i = 0; i < BATTERY_PROFILE_MAX; i++) {
		pr_info("profile=%d profile_rid=%d connected_rid=%d\n", i,
						chip->profile_rid[i],
						chip->connected_rid);
...
static int smb1360_fg_config(struct smb1360_chip *chip)
{
	...
	if (chip->fg_reset_at_pon) {
		...
		pr_info("v_predicted=%d v_now=%d reset_threshold=%d\n",
	...
	if (chip->batt_capacity_mah != -EINVAL
		...
		if (chip->batt_capacity_mah != -EINVAL) {
			...
			if (fcc_mah == chip->batt_capacity_mah) {
				pr_info("battery capacity correct\n");
			} else {
...
static void smb1360_check_feature_support(struct smb1360_chip *chip)
{
	...
	*/
	if (chip->revision == SMB1360_REV_1) {
		pr_info("FG config and Battery detection is not supported\n");
...
static int smb1360_enable(struct smb1360_chip *chip, bool enable)
{
	...
	if (!(val & SHDN_CMD_USE_BIT)) {
		pr_info("SMB not configured for CMD based shutdown\n");
		return 0;
	}
...
static int tn9010_batteryNotify_probe(struct platform_device *dev)    
{
...
struct platform_device TN9010_batteryNotify_device = {
    .name   = "tinno-battery",
    .id        = -1,
};

static struct platform_driver TN9010_batteryNotify_driver = {
    .probe        = tn9010_batteryNotify_probe,
    .driver     = {
        .name = "tinno-battery",
    },
};
...
static int smb_parse_dt(struct smb1360_chip *chip)
{
	...
	if (of_property_read_bool(node, "qcom,fg-reset-at-pon")) {
		...
		if (rc) {
			pr_info("FG reset voltage threshold not specified using 50mV\n");
...
//start add by xiongdajun 20140305	tinno charge temp manage	
	...
	chip->tinno_discharge_flag = false;
	// battery notofy UI
	rc = platform_device_register(&TN9010_batteryNotify_device);
	if (rc) {
		pr_info("****[batteryNotify] Unable to device register(%d)\n", rc);
		return rc;
	}    
	
	rc = platform_driver_register(&TN9010_batteryNotify_driver);
	if (rc) {
...
static int smb1360_resume(struct device *dev)
{
	...
	power_supply_changed(&chip->batt_psy);	//<BUG><JABALL-542><20150413>add uevent of state for battery;lishengli
	return 0;
}
...
//       u8 reg = 0;
	...
	if (rc)
      ...
      pr_info("smb1360_shutdown  shdn_after_pwroff=%d\n", chip->shdn_after_pwroff);
