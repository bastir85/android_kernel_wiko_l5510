...
static void wcd_mbhc_set_and_turnoff_hph_padac(struct wcd_mbhc *mbhc)
{
	...
	* removal event to sync-up PA's state */
	...
	//yangliang add for external padac for spk;20150708
	#if defined(CONFIG_PROJECT_L5510)
	if(current_ext_spk_pa_state == false)//yangliang add this condition external padac;20150708
	{	
		snd_soc_update_bits(codec, MSM8X16_WCD_A_ANALOG_RX_HPH_CNP_EN,
			    0x30, 0x00);
	}
	
	#elif defined(CONFIG_PROJECT_L5320) 
	if (!mbhc->is_speaker_active)
			snd_soc_update_bits(codec, MSM8X16_WCD_A_ANALOG_RX_HPH_CNP_EN,
			    0x30, 0x00);
...
static void wcd_mbhc_swch_irq_handler(struct wcd_mbhc *mbhc)
{
	...
	if ((mbhc->current_plug == MBHC_PLUG_TYPE_NONE) &&
		...
		//BEGIN <l5320> <DATA20150603> <l5320 project init> limi.zhang
			...
			if ( (!micbias1) && mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mb_source)//yangliang modify to cancle hph pulled out leads to secondary-mic tx on sound;20150529
		#else
			if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mb_source)
		#endif
