...
static atomic_t quat_mi2s_rsc_ref;//<BEGIN><FFBAKK-105><2014.12.15>add for tfa9890-pa;yangliang
static atomic_t quat_mi2s_clk_ref;//<BEGIN><FFBAKK-105><2015.02.02>add for tfa9890-pa-dsp mode;yangliang
#endif
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	case 0:
		if (atomic_read(&pdata->mclk_rsc_ref) <= 0)
			break;
		//msm8x16_wcd_mclk_enable(codec, 0, true);//mask for smartpa;yangliang
		mutex_lock(&pdata->cdc_mclk_mutex);
		...
		msm8x16_wcd_mclk_enable(codec, 0, true);//add for smartpa;yangliang
		mutex_unlock(&pdata->cdc_mclk_mutex);
		ret = pinctrl_select_state(pinctrl_info.pinctrl,
				pinctrl_info.cdc_lines_sus);
		if (ret < 0)
					...
					int enable,	bool dapm)
{
	...
	if (enable) {
		if (atomic_inc_return(&pdata->mclk_rsc_ref) == 1) {
			mutex_lock(&pdata->cdc_mclk_mutex);
			pdata->digital_cdc_clk.clk_val = pdata->mclk_freq;//add for smartpa;yangliang
...
static struct snd_soc_dai_link msm8x16_dai[] = {
	...
	{ /* hw:x,25 */  //<BEGIN><FFBAKK-105><2015.02.27>add for fm-speaker;yangliang
...
//<BEGIN><FFBAKK-105><2014.12.15>add for tfa9890-pa;yangliang
	...
	#endif
//<BEGIN><FFBAKK-105><2014.12.15>add for tfa9890-pa;
...
{
	//yangliang mask for not using int ret;
 	pr_debug("%s\n", __func__);
	#ifndef CONFIG_PROJECT_L9010
...
static int msm8x16_asoc_machine_probe(struct platform_device *pdev)
{
	...
	//yangliang add these 4-lines 0331
	...
	if (!strcmp(ptr, "external")) {
		...
		//bear_cards[pdev->id].name = dev_name(&pdev->dev);//mask for smartpa;yangliang
		card = &bear_cards[pdev->id];
	} else {
		card = &bear_cards[pdev->id];
		bear_cards[pdev->id].name = dev_name(&pdev->dev);
		//card = &bear_cards[pdev->id];//mask for smartpa;yangliang
