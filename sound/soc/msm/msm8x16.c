...
static int enable_spk_ext_pa(struct snd_soc_codec *codec, int enable)
{
	...
	//BEGIN <FDCAL-96> <20150720> <the speaker has no sound when plug off the earphone> panzaoyan
	#ifdef CONFIG_PROJECT_L5320
	struct msm8x16_wcd_priv *msm8x16_wcd = snd_soc_codec_get_drvdata(codec);
	#endif
	//END <FDCAL-96> <20150720> <the speaker has no sound when plug off the earphone> panzaoyan
	int ret = 0;
	...
	//2015/05/20 tyj. requested gpio before any operations to avoid gpio_ensure_requested warning in gpiolib.c.
	...
	//2015/05/10 tyj. changed enable/disable sequences based on aw8155(or other speaker pa in the future) datasheet.
	if(enable) {
		/*              _____________
		 *  mode 1:  __|                D class, multiple 8 
		 *              __   ________
		 *  mode 2:  __|  |_|           D class, multiple 12 
		 *             __    __   ___
		 *  mode 3:  __|  |_|  |_|      D class, multiple 12, JINGYIN(noise filter)
		 *
		 */
		//set up mode 2.
		ret += gpio_direction_output(pdata->spk_ext_pa_gpio, 1); 
		udelay(2);
		ret += gpio_direction_output(pdata->spk_ext_pa_gpio, 0);
		udelay(2);
		ret += gpio_direction_output(pdata->spk_ext_pa_gpio, 1);
	} else {
		ret = gpio_direction_output(pdata->spk_ext_pa_gpio, 0);		
...
//BEGIN <FDCAL-96> <20150720> <the speaker has no sound when plug off the earphone> panzaoyan
#ifdef CONFIG_PROJECT_L5320
	msm8x16_wcd->mbhc.is_speaker_active = enable;
#endif
//END <FDCAL-96> <20150720> <the speaker has no sound when plug off the earphone> panzaoyan
err:	
